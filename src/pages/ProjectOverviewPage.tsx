...
// ─── Animated ring gauge ─────────────────────────────────────────────────────
const HealthRing: React.FC<{ score: number; color: string }> = ({
  score,
  color,
}) => {
  const r = 52;
...
export const ProjectOverviewPage: React.FC = () => {
  ...
  if (loading)
    return <Loader message="Loading project intelligence dashboard..." />;
  if (!project)
    return <div className="p-10 text-text-muted">Project not found.</div>;
  ...
  const totalItems =
    inScopeItems.length + outOfScopeItems.length + uncertainItems.length;
  ...
  const docsUploaded = documents.filter(
    (d) => d.processing_status === "COMPLETED",
  ).length;
  const processingDocs = documents.filter(
    (d) =>
      d.processing_status === "PROCESSING" || d.processing_status === "PARSING",
  ).length;
  const initiationDocsCount = documents.filter(
    (d) => d.document_type === "EL" || d.document_type === "IFA",
  ).length;
  const monitoringDocsCount = documents.filter(
    (d) => d.document_type !== "EL" && d.document_type !== "IFA",
  ).length;

  // ── Risk severity breakdown ──────────────────────────────────────────────
  const riskHigh = openRisks.filter(
    (r) => r.risk_level === "HIGH" || r.risk_level === "CRITICAL",
  ).length;
  ...
  return (
    <div className="flex-1 bg-bg-base text-text-primary p-6 md:p-8 min-h-screen overflow-auto">
      ...
      <div className="max-w-7xl mx-auto space-y-6 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <Link
                to="/projects"
                className="text-[11px] text-[#4A4A4A] dark:text-gray-400 hover:text-[#FF5A14] font-medium transition-colors"
              >
                Projects Directory
              </Link>
              <ChevronRight className="w-3 h-3 text-[#4A4A4A] dark:text-gray-500" />
              <span className="text-[11px] text-[#FF5A14] font-bold">
                Project Overview
              </span>
            </div>
            ...
            <p className="text-[#4A4A4A] dark:text-text-muted text-xs mt-0.5 font-medium">
              {project.client_name} &nbsp;·&nbsp;{" "}
            ...
            <Link
              ...
              className={
                isDark
                  ...
                  : "inline-flex items-center gap-1.5 px-3.5 py-2 bg-[#FFF7F2] hover:bg-[#FFEFE5] border border-[#D8D8D8] text-[#333333] text-xs font-extrabold rounded-xl transition-all shadow-sm"
              }
            >
              <UploadCloud className="w-3.5 h-3.5 text-[#FF5A14]" /> Upload
              Documents
            </Link>
            <Link
              ...
              className={
                isDark
                  ...
                  : "inline-flex items-center gap-1.5 px-3.5 py-2 bg-[#F3E8FF] hover:bg-[#E9D5FF] border border-[#A855F7] text-[#3B0764] text-xs font-black rounded-xl transition-all shadow-sm"
              }
            >
              <ScrollText
                className={`w-3.5 h-3.5 ${isDark ? "text-purple-300" : "text-[#3B0764]"}`}
              />{" "}
              Baseline Review
            </Link>
            <Link
              ...
              className={
                isDark
                  ...
                  : "inline-flex items-center gap-1.5 px-3.5 py-2 bg-[#FFE4E6] hover:bg-[#FECDD3] border border-[#F43F5E] text-[#4C0519] text-xs font-black rounded-xl transition-all shadow-sm"
              }
            >
              <Activity
                className={`w-3.5 h-3.5 ${isDark ? "text-rose-300" : "text-[#4C0519]"}`}
              />{" "}
              Risk Tracker
            </Link>
            <Link
              ...
              className={
                isDark
                  ...
                  : "inline-flex items-center gap-1.5 px-3.5 py-2 bg-[#FFF7F2] hover:bg-[#FFEFE5] border border-[#FF8A55] text-[#FF5A14] text-xs font-extrabold rounded-xl transition-all shadow-sm"
              }
            >
        ...
        {/* ── RAG BANNER ──────────────────────────────────────────────── */}
        <div
          className={`flex items-center gap-3 px-5 py-3 rounded-2xl ${rag.bg} border ${rag.border}`}
        >
          <span
            className={`text-xs font-black uppercase tracking-widest ${rag.labelColor} flex items-center gap-2`}
          >
            <span
          ...
          <span className="text-xs text-[#333333] dark:text-text-muted flex-1 font-medium leading-relaxed">
            {rag.label === "SETTING UP"
          ...
          <span className={`text-lg font-black ${rag.text}`}>
            {healthScore}%
          </span>
        </div>
          ...
          <div className="col-span-12 md:col-span-3 bg-bg-card/50 border border-border-subtle rounded-2xl p-5 flex flex-col items-center justify-center text-center backdrop-blur-md relative z-10 hover:z-30 transition-all">
                    ...
                    <div>
                      ...
                      <ul className="list-disc pl-4 space-y-0.5 text-text-muted font-medium">
                        ...
                        <li>
                          ...
                          Baseline approved (20 pts if extracted but pending
                          review).
                        </li>
                      </ul>
                    </div>
                    ...
                    <div>
                      ...
                      <ul className="list-disc pl-4 space-y-0.5 text-text-muted font-medium">
                        ...
                        <li>
                          Deducts up to{" "}
                          <strong className="text-text-primary">60 pts</strong>{" "}
                          based on the proportion of unresolved risks.
                        </li>
                        <li>
                          Deducts up to{" "}
                          <strong className="text-text-primary">40 pts</strong>{" "}
                          for timeline delays (40 pts if past end date, 20 pts
                          if within 14 days of end date).
                        </li>
                      </ul>
                    </div>
                  </div>
            ...
            <div className="relative">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className={`text-3xl font-black ${rag.text}`}>
                  {healthScore}
                </span>
            ...
            <span
              className={`mt-2 text-xs font-black tracking-widest uppercase ${rag.labelColor}`}
            >
              {rag.label}
            </span>
                      ...
                      },
                      {
                        ...
                        points:
                          risks.length === 0
                            ...
                            : openRisks.length === 0
                              ? "OK"
                              : `-${Math.min(60, Math.round((openRisks.length / risks.length) * 60))}pts`,
          ...
          <div className="col-span-12 md:col-span-9 grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-bg-card/50 border border-border-subtle rounded-2xl p-5 backdrop-blur-md flex flex-col justify-between">
              <div className="flex justify-between items-start mb-2">
                <span className="text-[10px] font-bold uppercase tracking-wider text-[#4A4A4A] dark:text-text-muted">
                  Documents
                </span>
              ...
              <div>
                ...
                <div className="space-y-1.5 text-[11px] border-t border-border-subtle pt-2">
                  <div className="flex justify-between">
                    <span className="text-[#4A4A4A] dark:text-text-muted font-medium">
                      Contract Initiation
                    </span>
            ...
            <div className="bg-bg-card/50 border border-border-subtle rounded-2xl p-5 backdrop-blur-md flex flex-col justify-between">
              ...
              <div>
                <p
                  className={`text-3xl font-black mb-1 ${openRisks.length > 0 ? "text-rose-500 dark:text-rose-400" : "text-emerald-400"}`}
                >
                  {openRisks.length}
                </p>
                <div className="space-y-0.5 text-[11px]">
                  <div className="flex justify-between">
                    ...
                    <span
                      className={`font-semibold ${criticalRisks.length > 0 ? "text-rose-500 dark:text-rose-400" : "text-[#4A4A4A] dark:text-text-muted"}`}
                    >
            ...
            <div className="bg-bg-card/50 border border-border-subtle rounded-2xl p-5 backdrop-blur-md flex flex-col justify-between">
              ...
              <div>
                <p
                  className={`text-3xl font-black mb-1 ${baselineApproved ? "text-emerald-400" : "text-amber-500 dark:text-amber-400"}`}
                >
                  {totalItems}
                </p>
                <div className="space-y-0.5 text-[11px]">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span
                      className={`font-semibold ${baselineApproved ? "text-emerald-400" : "text-amber-500 dark:text-amber-400"}`}
                    >
                      {baselineApproved
                        ? "Approved"
                        : baseline
              ...
              <div>
                {daysRemaining !== null ? (
                  <>
                    ...
                    <p className="text-[11px] text-[#4A4A4A] dark:text-text-muted font-medium">
                ...
                {project.start_date && (
                  <p className="text-[10px] text-[#4A4A4A] dark:text-gray-400 mt-1 font-medium">
                    Started{" "}
                ...
                <div className="flex-1 space-y-0.5 text-[11px]">
                  <div className="flex justify-between">
                    ...
                    <span className="text-indigo-400 font-semibold">
                      {members.filter((m) => m.role === "STAKEHOLDER").length}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[#4A4A4A] dark:text-text-muted font-medium">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-[#4A4A4A] dark:text-text-muted font-semibold">
                      {
                        members.filter(
                          (m) =>
                            m.role !== "STAKEHOLDER" && m.role !== "TEAM_LEAD",
                        ).length
                      }
                    </span>
                  </div>
            ...
            <div className="col-span-2 lg:col-span-2 bg-bg-card/50 border border-border-subtle rounded-2xl p-5 backdrop-blur-md flex flex-col justify-between">
              ...
              <div className="flex items-end gap-4">
                <div>
                  <span
                    className={`inline-flex items-center gap-1.5 text-xs font-black px-3 py-1.5 rounded-full uppercase tracking-wider ${
                      project.monitoring_status === "ACTIVE"
                        ? "bg-emerald-500/15 text-emerald-400 border border-emerald-500/30"
                        : project.monitoring_status === "DRAFT"
                          ? "bg-gray-500/15 text-[#4A4A4A] dark:text-text-muted border border-gray-500/30"
                  ...
                  <p className="text-[11px] text-[#4A4A4A] dark:text-text-muted mt-2 font-medium">
                    {project.monitoring_status === "ACTIVE"
          ...
          <div className="bg-bg-card/50 border border-border-subtle rounded-2xl p-5 backdrop-blur-md flex flex-col justify-between h-full">
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <Link
                  to={`/projects/${id}/cockpit`}
                  className="text-[11px] text-[#FF5A14] hover:opacity-80 flex items-center gap-1 font-bold"
                >
              ...
              <div className="space-y-2">
                {[
                  ...
                  {
                    label: "Independence & Financial (IFA)",
                    type: "IFA",
                    present: hasIFA,
                    section: "Initiation",
                  },
          ...
          <div className="bg-bg-card/50 border border-border-subtle rounded-2xl p-5 backdrop-blur-md flex flex-col justify-between h-full">
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <Link
                  to={`/projects/${id}/baseline`}
                  className="text-[11px] text-purple-500 dark:text-purple-400 hover:text-purple-600 dark:text-purple-300 flex items-center gap-1 font-semibold"
                >
          ...
          <div className="bg-bg-card/50 border border-border-subtle rounded-2xl p-5 backdrop-blur-md flex flex-col justify-between h-full">
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <Link
                  to={`/projects/${id}/tracker`}
                  className="text-[11px] text-rose-500 dark:text-rose-400 hover:text-rose-600 dark:text-rose-300 flex items-center gap-1 font-semibold"
                >
            ...
            {members.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-[280px] overflow-y-auto pr-1">
                {members.map((member) => (
                  <div
                    key={member.id}
                    className="flex items-center gap-3 p-3 bg-bg-hover/50 rounded-xl border border-border-strong/30 hover:border-teal-500/30 transition-all"
                  >
                    <div
              ...
              {[
                ...
                <div className="py-8 text-center">
                  ...
                  <p className="text-xs text-text-muted mt-1">
                    No immediate actions required. Project is healthy.
                  </p>
                </div>
              )}
            </div>
          ...
          {[
            {
              ...
              desc: "Upload & manage project documents",
              to: `/projects/${id}/cockpit`,
              icon: (
                <UploadCloud
                  className={`w-5 h-5 ${
              ...
              color: isDark
                ? "bg-gradient-to-br from-blue-600/20 to-blue-500/5 border-blue-500/20 text-blue-300"
                : "bg-blue-50/80 hover:bg-blue-100/80 border-blue-200 text-blue-900 shadow-sm",
            },
            {
              label: "Baseline Review",
              desc: "Scope items & approval workflow",
              to: `/projects/${id}/baseline`,
              icon: (
                <ScrollText
                  className={`w-5 h-5 ${
              ...
              color: isDark
                ? "bg-gradient-to-br from-purple-600/20 to-purple-500/5 border-purple-500/20 text-purple-300"
                : "bg-purple-50/80 hover:bg-purple-100/80 border-purple-200 text-purple-900 shadow-sm",
            },
            {
              label: "Risk Tracker",
              desc: "Monitor & resolve scope risks",
              to: `/projects/${id}/tracker`,
              icon: (
                <Activity
                  className={`w-5 h-5 ${
              ...
              color: isDark
                ? "bg-gradient-to-br from-rose-600/20 to-rose-500/5 border-rose-500/20 text-rose-300"
                : "bg-rose-50/80 hover:bg-rose-100/80 border-rose-200 text-rose-900 shadow-sm",
            },
            {
              label: "Team Members",
              desc: "Stakeholders & team leads",
              to: `/projects/${id}/members`,
              icon: (
                <Users
                  className={`w-5 h-5 ${
              ...
              color: isDark
                ? "bg-gradient-to-br from-teal-600/20 to-teal-500/5 border-teal-500/20 text-teal-300"
                : "bg-teal-50/80 hover:bg-teal-100/80 border-teal-200 text-teal-900 shadow-sm",
            },
          ].map((item) => (
            <Link
              key={item.to}
              to={item.to}
              className={`${item.color} border rounded-2xl p-4 hover:scale-[1.02] transition-all duration-200 group`}
            >
              ...
              <p
                className={`text-xs font-black mb-0.5 ${
                  isDark ? "text-text-primary" : "text-[#111827]"
                }`}
              >
                {item.label}
              </p>
              <p
                className={`text-[10px] font-semibold ${
                  isDark ? "text-text-muted" : "text-[#374151]"
                }`}
              >
                {item.desc}
              </p>
