      ...
      <div className="max-w-6xl mx-auto space-y-8 relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((stat, i) => (
            <div 
              key={i} 
              className={`p-6 rounded-2xl bg-gradient-to-br ${stat.bg} border border-border-subtle backdrop-blur-sm flex flex-col justify-between`}
            >
              ...
              <div>
                ...
                <div className="flex items-center gap-1 text-[10px] text-text-muted font-medium">
                  <TrendingUp className="w-3 h-3 text-teal-500 dark:text-teal-400" />
                  {stat.change}
                </div>
              </div>
            </div>
              ...
              <div className="space-y-4">
                {loading ? (
                  ...
                  data.recent_activities.map((log, idx) => (
                    <div key={idx} className="flex gap-4 p-3 rounded-xl hover:bg-bg-card transition-colors group">
                      <div className="mt-1">
