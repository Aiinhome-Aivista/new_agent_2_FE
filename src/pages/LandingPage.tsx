...
export const LandingPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-bg-base text-text-primary flex flex-col relative overflow-hidden">
      ...
      {/* Header */}
      <header className="relative z-10 w-full max-w-7xl mx-auto px-6 py-6 flex justify-between items-center border-b border-border-subtle backdrop-blur-sm">
        ...
        <div>
          {user ? (
            ...
            <Link 
              to="/login" 
              className="px-5 py-2.5 bg-bg-hover hover:bg-white/10 border border-border-strong rounded-xl font-medium transition-all duration-300 hover:border-white/20"
            >
              Sign In
            </Link>
          )}
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center text-center px-6 py-16 md:py-24 max-w-7xl mx-auto w-full">
        {/* Badge */}
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-teal-500/10 border border-teal-500/25 text-teal-600 dark:text-teal-300 text-sm font-semibold mb-8 animate-pulse">
        ...
        <div className="mb-20">
          {user ? (
            <Link 
              to="/dashboard" 
              className="group inline-flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-teal-500 to-blue-600 hover:from-teal-400 hover:to-blue-500 text-text-primary font-semibold rounded-2xl transition-all duration-300 shadow-xl shadow-teal-500/20 hover:shadow-teal-400/30 hover:-translate-y-0.5"
            >
              Manage Projects
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full text-left relative z-20">
          {[
            { 
              icon: <FileText className="w-6 h-6 text-teal-500 dark:text-teal-400" />,
            ...
            <div 
              key={idx} 
              className="group p-8 rounded-2xl bg-bg-hover border border-border-subtle hover:border-teal-500/30 hover:bg-white/[0.05] transition-all duration-300 hover:shadow-2xl hover:shadow-teal-500/5 hover:-translate-y-1 relative overflow-hidden"
            >
              ...
              <div className="p-3 bg-bg-hover group-hover:bg-teal-500/10 rounded-xl w-fit mb-5 transition-colors duration-300">
                {feature.icon}
              </div>
              <h3 className="font-display text-xl font-bold mb-3 text-text-primary group-hover:text-teal-600 dark:text-teal-300 transition-colors duration-300">
                {feature.title}
              </h3>
